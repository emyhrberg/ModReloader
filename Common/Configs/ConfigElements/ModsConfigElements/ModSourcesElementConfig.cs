...
namespace ModReloader.Common.Configs.ConfigElements.ModsConfigElements
{
    // Contains:
    // Icon image
    // Mod name
    ...
    public class ModSourcesElementConfig : UIPanel
    {
        ...
        public ModSourcesElementConfig(ModSourcesConfig parent, string fullModPath, string cleanName = "")
        {
            ...
            if (isModEnabled)
            {
                ...
                ConfigIcon modConfigIcon = new(texture: Ass.ConfigOpen, modPath: internalNameFolderName, hover: $"Open config", cleanModName: cleanModName);
                float size = 22f;
            ...
            // reload icon
            ReloadIcon modReloadIcon = new(Ass.ModReload.Value, internalNameFolderName, "Reload " + internalNameFolderName);
            modReloadIcon.Left.Set(def - dist * 2, 1f);
            Append(modReloadIcon);

            // folder icon
            FolderIcon folderIcon = new(Ass.ModOpenFolder.Value, fullModPath, "Open Folder");
            folderIcon.Left.Set(def - dist * 1, 1f);
            Append(folderIcon);

            // cs proj icon
            OpenProjectIcon projectIcon = new(Ass.ModOpenProject.Value, fullModPath, "Open .csproj");
